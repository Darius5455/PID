...
namespace Framework.ViewModel
{
    public class MenuCommands : BaseVM
    {
        ...
        #region Linear Operator
        private ICommand _linearOpCommand;
        public ICommand LinearOpCommand
        {
            get
            {
                if (_linearOpCommand == null)
                    _linearOpCommand = new RelayCommand(LinearOpImg);
                return _linearOpCommand;
            }
        }

        private void LinearOpImg(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Please add an image!");
            ...
            DialogWindow diag = new DialogWindow(_mainVM, labels);
            diag.ShowDialog();

            List<double> values = diag.GetValues();
            ...
            if (alpha <= 0)
            {
                MessageBox.Show("Alpha must be positive!");
                return;
            }

            ClearProcessedCanvas(parameter as Canvas);

            var lut = PointwiseOperations.CreateLinearOperatorLUT(alpha, beta);
        ...
        private ICommand _gammaOpCommand;
        public ICommand GammaOpCommand
        {
            get
            {
                if (_gammaOpCommand == null)
                    _gammaOpCommand = new RelayCommand(GammaOpImg);
                return _gammaOpCommand;
            }
        }

        private void GammaOpImg(object parameter)
        {
            if (InitialImage == null)
            {
                MessageBox.Show("Please add an image!");
            ...
            if (gamma <= 0)
            {
                MessageBox.Show("Gamma must be positive!");
                return;
            }

            ClearProcessedCanvas(parameter as Canvas);

            var lut = PointwiseOperations.CreateGammaOperatorLUT(gamma);
        ...
        private void BinaryImage(object parameter)
        {
            ...
            DialogWindow dialog = new DialogWindow(_mainVM, new System.Collections.Generic.List<string> { "Threshold T (10 - 145):" });
            dialog.ShowDialog();

            var values = dialog.GetValues();
            double tValue = values[0];

            if (tValue < 10 || tValue > 145)
            {
        ...
        private ICommand _colorContrastStretchingCommand;
        public ICommand ColorContrastStretchingCommand
        {
            get
            {
                if (_colorContrastStretchingCommand == null)
                    _colorContrastStretchingCommand = new RelayCommand(ColorContrastStretching);
                return _colorContrastStretchingCommand;
            }
        }

        private void ColorContrastStretching(object parameter)
        {
            if (ColorInitialImage == null)
            {
                MessageBox.Show("Please load a color image!");
                return;
            }

            ClearProcessedCanvas(parameter as Canvas);

            ColorProcessedImage = PointwiseOperations.ContrastStretchingHSV(ColorInitialImage);
            ProcessedImage = Convert(ColorProcessedImage);
        }
        #endregion

        #region Convert RGB to HSV
        private ICommand _convertRGBtoHSVCommand;
        public ICommand ConvertRGBtoHSVCommand
        {
            get
            {
                if (_convertRGBtoHSVCommand == null)
                    _convertRGBtoHSVCommand = new RelayCommand(ConvertRGBtoHSV);
                return _convertRGBtoHSVCommand;
            }
        }

        private void ConvertRGBtoHSV(object parameter)
        {
            if (ColorInitialImage == null)
            {
                MessageBox.Show("Please load a color image!");
            ...
            ColorProcessedImage = PointwiseOperations.ConvertRGBtoHSV(ColorInitialImage);
            ProcessedImage = Convert(ColorProcessedImage);
        }
        #endregion
